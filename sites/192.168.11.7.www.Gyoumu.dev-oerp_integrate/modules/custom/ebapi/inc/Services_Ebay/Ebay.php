...
class Services_Ebay
{
  ...
  /**
   * authentication based on SessionID
   */
   const AUTH_TYPE_SESSION  = 3;

  /**
   * authentication based on RuName
   */
   const AUTH_TYPE_RUNAME  = 4;
    ...
    */
    ...
    {
      return new EbaySession($devId, $appId, $certId, $encoding);
    }

    public static function getSandboxSession($devId, $appId, $certId, $encoding = 'ISO-8859-1')
    {
      return new EbaySandboxSession($devId, $appId, $certId, $encoding);
    }
...
class eBayDateHelper
{
  const format = 'Y-m-d\TH:i:s.000\Z';

  static private function getDateObj($date)
  {
  ...
  static public function getMostFromDate($date)
  {
    $Date = self::getDateObj($date);
    $Date->modify('-120day +1sec');
    return $Date->format(self::format);
  }
}
?>