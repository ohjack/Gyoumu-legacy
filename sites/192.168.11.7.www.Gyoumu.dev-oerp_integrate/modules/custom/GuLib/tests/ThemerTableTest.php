...
class ThemerTableTest extends PHPUnit_Framework_TestCase
{
  ...
  {
    $ProcHds = new \Gulei\Processor\Basic(array('isHds' => true));
    $ProcRecs = new \Gulei\Processor\Basic(array('isRecs' => true));
  ...
  {
    $ProcHds = new \Gulei\Processor\Basic();
    $ProcHds->setDataProvider(
      new G\\Gulei\Provider\Lambda(array('f1'))
    );

    $ProcRecs = new \Gulei\Processor\Basic();
    $ProcRecs->setDataProvider(
      new G\\Gulei\Provider\Lambda(array(array('f1' => 'Field 1')))
    );
